...
class LogisticRegression(object):
    ...
    def __init__(self, theta=None):
        ...
        self.PATIENCE = 10                   # A max number of consequent epochs with monotonously
    ...
    def conditional_log_prob(self, label, datapoint):
        ...
        # YOUR CODE HERE
        #
        theta_x = np.matmul(np.transpose(self.theta), np.transpose(datapoint))
        softmax = np.exp(theta_x) / np.reshape(np.sum(np.exp(theta_x), axis=0), (1, -1))
    ...
    def compute_gradient(self, minibatch):
        ...
        gradient = np.matmul(np.transpose(self.x[minibatch]), np.transpose((softmax+c))) / len(minibatch)
        return gradient
    ...
    def fit(self, x, y):
        ...
        while count < self.PATIENCE and i < 5000:
            idx = random.sample(range(len(self.x)), min(self.MINIBATCH_SIZE, len(self.x)))
    ...
    def classify_word(self, x):
        ...
        theta_x = np.matmul(np.transpose(self.theta), np.reshape(x, (-1, 1)))
        softmax = np.exp(theta_x) / np.reshape(np.sum(np.exp(theta_x), axis=0), (1, -1))
        return np.reshape(np.log(softmax), (1, -1))
