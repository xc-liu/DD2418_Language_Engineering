...
class BigramTester(object):
    ...
    def compute_entropy_cumulatively(self, word):
        # YOUR CODE HERE
        try:
            bi_prob = np.exp(self.bigram_prob[self.last_index][self.index[word]])
        except:
            bi_prob = 0

        try:
            uni_prob = self.unigram_count[self.index[word]] / self.total_words
            self.last_index = self.index[word]
        except:
            uni_prob = 0
            self.last_index = -1

        new_log = -math.log(self.lambda1 * bi_prob + self.lambda2 * uni_prob + self.lambda3)
        self.logProb = (self.logProb * self.test_words_processed + new_log) / (self.test_words_processed + 1)
