...
class NERClassifier(nn.Module):
    def __init__(self, word_emb_file, max_word_length, char_emb_size=16, char_hidden_size=25, word_hidden_size=100,
    ...
    def forward(self, x):
        ...
        unknown_idx = self.w2i.get(UNKNOWN_WORD)
        padding_idx = self.w2i.get(PADDING_WORD)
        glove_idx = torch.tensor([[self.w2i.get(w.lower(), unknown_idx) if w != PADDING_WORD else padding_idx for w in s] for s in x])
        glove_embedding = self.word_emb(glove_idx)

        if self.char_emb_size > 0:
            batch_size = len(x)
            max_sentence_length = len(x[0])
            unknown_word = self.c2i.get(UNKNOWN_WORD)
            char_embedding = torch.tensor(
                [[[[self.c2i[c] for c in w] + [unknown_word] * (self.max_word_length - len(w))] for w in s] for s in x])
            char_embedding = self.char_emb(char_embedding)
            char_embedding = torch.reshape(char_embedding,
                                           (batch_size * max_sentence_length, self.max_word_length, self.char_emb_size))
            if cud:
                _, h_fw = self.char_birnn.forward(char_embedding)
                char_vector = torch.reshape(h_fw, (batch_size, max_sentence_length, self.char_hidden_size))
            else:
                _, h_fw, h_bw = self.char_birnn.forward(char_embedding)
                char_vector = torch.transpose(torch.cat((h_fw, h_bw), dim=0), 0, 1)
                char_vector = torch.reshape(char_vector, (batch_size, max_sentence_length, 2 * self.char_hidden_size))

            word_embedding = torch.cat((glove_embedding, char_vector), dim=2)

        else:
            word_embedding = glove_embedding

        if wud:
            word_vector, _ = self.word_birnn.forward(word_embedding)
        else:
            word_vector, _, _ = self.word_birnn.forward(word_embedding)

        outputs = self.final_pred(word_vector)
        return outputs
...
if __name__ == '__main__':
    ...
    wud = args.word_unidirectional
    cud = args.char_unidirectional
    max_word_length = 0
    for sentence in training_data.sentences:
