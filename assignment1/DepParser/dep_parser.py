...
class Parser: 
    ...
    def step_by_step(self,string) :
        ...
        while True :
            ...
            try :
                ...
                print("valid moves: ", self.valid_moves(i, stack, pred_tree))
                print( "Illegal move" )
                continue
    ...
    def compute_correct_move(self, i,stack,pred_tree,correct_tree) :
        ...
        # YOUR CODE HERE
        v_moves = self.valid_moves(i, stack, pred_tree)
        ...
        if len(v_moves) == 1:
            return self.SH
        if len(v_moves) == 2:
            if correct_tree[stack[-1]] == stack[-2]:
                return self.RA
            elif correct_tree[stack[-2]] == stack[-1]:
                return self.LA
            else:
                print("Invalid tree!")
        if len(v_moves) == 3:
            right_futureRef = False
            left_futureRef = False
            for j in range(i, len(correct_tree)):
                if correct_tree[j] == stack[-1]:
                    right_futureRef = True
                if correct_tree[j] == stack[-2]:
                    left_futureRef = True
            if not right_futureRef and correct_tree[stack[-1]] == stack[-2]:
                return self.RA
            if not left_futureRef and correct_tree[stack[-2]] == stack[-1]:
                return self.LA
            else:
                return self.SH
        else:
...
move_filename = Path("moves_en-ud-dev.conllu")
correct_move_filename = Path("correct_moves_en-ud-dev.conllu")
...
if __name__ == '__main__':
    ...
    if args.step_by_step:
        ...
        with open(args.compute_correct_moves) as source, open(correct_move_filename, "r") as correct_answer:
            total_count = 0
            correct_count = 0
            for w,tags,tree,relations in p.trees(source) :
                moves = p.compute_correct_moves(tree)
                correct_moves = correct_answer.readline()
                correct_moves = (correct_moves[1:-2]).split(",")
                correct_moves = [int(correct_moves[i]) for i in range(len(correct_moves))]
                total_count += 1
                if correct_moves == moves:
                    correct_count += 1
